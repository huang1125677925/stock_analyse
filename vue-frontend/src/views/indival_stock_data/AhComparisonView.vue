  ...
  <el-card class="page-card">
    ...
    <el-form :inline="true" :model="query" class="query-form">
      ...
      <el-form-item label="A股代码">
        <el-input v-model="query.ts_code" placeholder="例如 600519.SH" clearable />
      </el-form-item>
      <el-form-item label="港股代码">
        <el-input v-model="query.hk_code" placeholder="例如 00960.HK" clearable />
      </el-form-item>
...
/**
 * 组件：AhComparisonView
 * 功能：提供 AH 股比价数据的查询与展示（单一职责：仅处理 ah-comparison 接口）
 * 参数：无（通过内部表单收集查询参数：ts_code、hk_code、start_date、end_date）
 * 返回值：无（渲染后展示表格数据，不向外部返回值）
 * 事件：
 *  - 查询按钮点击（onSearch）：发起接口请求并更新表格
 *  - 重置按钮点击（onReset）：清空查询条件与结果
 */

const loading = ref(false)
const dateRange = ref<[string, string] | null>(null)
const quickRange = ref<string>('last7d')
const query = ref<AhComparisonQueryParams>({})
...
async function onSearch() {
  loading.value = true
  try {
    const params: AhComparisonQueryParams = { ...query.value }
    if (dateRange.value) {
