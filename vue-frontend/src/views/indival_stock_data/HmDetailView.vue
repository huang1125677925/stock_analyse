<template>
  <el-card class="page-card">
    <template #header>
      <div class="card-header">游资每日明细</div>
    </template>
    ...
    <el-form :inline="true" :model="query" class="query-form">
      ...
      <el-form-item label="游资名称">
        <el-input v-model="query.hm_name" placeholder="如 上海某营业部" clearable />
      </el-form-item>
      <el-form-item label="股票代码">
        <el-input v-model="query.ts_code" placeholder="如 000001.SZ" clearable />
      </el-form-item>
...
import { fetchHmDetail, type HmDetailQueryParams } from '@/services/hmDetailApi'

/**
 * 组件：HmDetailView
 * 功能：查询并展示游资每日交易明细（单一职责：仅处理 hm-detail 接口）
 * 参数：无（内部表单：hm_name、ts_code、start_date、end_date）
 * 返回值：无
 * 事件：
 *  - onSearch：发起查询
 *  - onReset：重置状态
...
const query = ref<HmDetailQueryParams>({})
...
async function onSearch() {
  loading.value = true
  try {
    const params: HmDetailQueryParams = { ...query.value }
    if (dateRange.value) {
      params.start_date = dateRange.value[0]
      params.end_date = dateRange.value[1]
    }
    const data = await fetchHmDetail(params)
