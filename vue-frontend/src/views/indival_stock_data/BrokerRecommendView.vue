<template>
  <el-card class="page-card">
    <template #header>
      <div class="card-header">券商月度金股</div>
    </template>
...
import { fetchBrokerRecommend, type BrokerRecommendQueryParams } from '@/services/brokerRecommendApi'

/**
 * 组件：BrokerRecommendView
 * 功能：查询并展示券商月度金股数据（单一职责：仅处理 broker-recommend 接口）
 * 参数：无（内部表单：日期范围，内部映射为 month 参数）
 * 返回值：无
 * 事件：
 *  - 查询点击：onSearch
 *  - 重置点击：onReset
...
const query = ref<BrokerRecommendQueryParams>({ month: '' })
...
async function onSearch() {
  loading.value = true
  try {
    // 将日期范围的结束日期映射为月份参数（YYYYMM），以符合后端接口
    const month = dateRange.value ? (dateRange.value[1]?.slice(0, 6) || '') : ''
    const data = await fetchBrokerRecommend({ month })
...
onMounted(() => {
  // 页面挂载默认设置最近一周，并自动触发查询（内部将结束日期映射为 month）
