...
const fetchIndexList = async () => {
  ...
  try {
    ...
    if (searchForm.search.trim()) {
      params.append('search', searchForm.search.trim())
    }
    if (searchForm.code.trim()) {
      params.append('code', searchForm.code.trim())
    }

    const response = await axios.get<ApiResponse>(`/django/api/market/index-basic-data/?${params}`)
    
    if (response.data.code === 200) {
      indexList.value = response.data.data.list
...
const viewIndexDetail = (row: IndexItem) => {
  ElMessage.info(`查看指数详情: ${row.name} (${row.code})`)
  // TODO: 跳转到指数详情页面或打开详情弹窗
}
