      ...
      <template #header>
        <div class="card-header">
          <div class="header-title">
            <span>{{ query.industry_name ? `${query.ts_code} - ${query.industry_name} - ${metricOptions.find(opt => opt.value === selectedMetric)?.label || selectedMetric} 趋势` : '申万行业日线行情趋势' }}</span>
      ...
      <div class="form-row">
        <el-form :inline="true" label-width="80px">
          <el-form-item label="行业代码">
            <el-input v-model="query.ts_code" placeholder="输入行业代码" style="width: 150px" />
          </el-form-item>
...
function updateChart(data: DailyRecord[] = records.value) {
  ...
  const option: echarts.EChartsOption = {
    // title: {
    //   text: query.industry_name ? `${query.ts_code} - ${query.industry_name} - ${metricLabel} 趋势` : `${query.ts_code} - ${metricLabel} 趋势`,
    //   left: 'center'
    // },
    tooltip: {
      trigger: 'axis',
  ...
  chartInstance.setOption(option, true) // 使用 true 选项合并模式，或者完全重置
}
