...
import { HEATMAP_METRICS, HeatmapMetricType } from '@/services/industry-heatmap'
import type { IndustryHeatmapData, IndustryHeatmapDataItem } from '@/services/industry-heatmap'

interface Props {
  /** 行业业绩数据 */
  data: IndustryHeatmapData | null
  /** 选择的业绩指标 */
...
const emit = defineEmits<{
  ...
  'chart-click': [payload: { industry?: { indexCode: string; indexName: string }, date?: string, value?: number, metric: HeatmapMetricType, rawParams: any }]
}>()
...
const processedIndustries = computed(() => {
  ...
  return [...filteredIndustries].sort((a, b) => {
    ...
    const aLastValue = aData[aData.length - 1][props.selectedMetric as keyof IndustryHeatmapDataItem] as number
    const bLastValue = bData[bData.length - 1][props.selectedMetric as keyof IndustryHeatmapDataItem] as number
...
const chartOption = computed((): echarts.EChartsOption => {
  ...
  industries.forEach((industry, industryIndex) => {
    ...
    if (industryData) {
      industryData.forEach((item: IndustryHeatmapDataItem, dateIndex: number) => {
        const value = item[props.selectedMetric as keyof IndustryHeatmapDataItem] as number
        // 检查value是否为有效数字
  ...
  // 获取指标配置
  const metricConfig = HEATMAP_METRICS[props.selectedMetric]
